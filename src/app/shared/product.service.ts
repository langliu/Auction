...
export class ProductService {
  ...
  getProducts(): Observable<Product[]> {
    return this.http.get(`${this.url}/api/products`).map(res => res.json());
  }
  ...
  getCommentsForProductId(id: number): Observable<Comment[]> {
    return this.http
      .get(`${this.url}/api/product/${id}/comments`)
      .map(res => res.json());
  }
  ...
  search(params: ProductSearchParams): Observable<Product[]> {
    return this.http
      .get(`${this.url}/api/products`, { search: this.encodeParams(params) })
      .map(res => res.json());
  }
  ...
  encodeParams(params: ProductSearchParams) {
    return Object.keys(params)
      ...
      .reduce((param: string, key: string) => {
        if (param) {
